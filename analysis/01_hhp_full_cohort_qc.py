...
def choose_best_model_for_series(x_time_years, y_diameter_cm, bic_threshold=-2.0, require_cv_improvement=True):
    ...
    # Adjusted R^2 is undefined when df_resid <= 0 (e.g., n=2 for linear OLS)
    out["lin_adj_r2"] = float(lin.rsquared_adj) if getattr(lin, "df_resid", 0) > 0 else float("nan")
    ...
    if n == 2:
        lin = _fit_linear_ols(x, y)
        out["lin_slope_cm_per_year"] = float(lin.params[1]) if len(lin.params) > 1 else float("nan")
        out["chosen_model"] = "linear"
        out["chosen_slope_cm_per_year"] = out["lin_slope_cm_per_year"]
        out["chosen_avg_slope_cm_per_year_over_span"] = float(out["lin_slope_cm_per_year"])
    return out
...
# Input / output paths
INPUT_CSV = "aorta_data_long_from_AllHHP2025.csv"
...
def mixedlm_predict_conditional(mdf, df, group_col):
    ...
    Notes:
      * Patsy may drop rows with missing values in model terms when constructing the design matrix.
        To keep alignment, we return an array of length len(df) with NaN for rows that could not
        be scored (e.g., missing height_cm or time_years).
      * Predictions are conditional on each group’s BLUPs (random effects) when available.
    """
    import numpy as np
    ...
    pos_map = {ix: i for i, ix in enumerate(df_index)}

    # Build design matrix for rows patsy can score (may drop rows with NA in model terms)
    ...
    fixed = (X.to_numpy() @ fe.to_numpy()).astype(float)

    preds_full = np.full(len(df), np.nan, dtype=float)

    used_idx = list(X.index)
    for j, ix in enumerate(used_idx):
        preds_full[pos_map[ix]] = fixed[j]

    re_dict = mdf.random_effects

    # Add random effects only for rows that were scored by patsy
    for j, ix in enumerate(used_idx):
        g = df.loc[ix, group_col]
        re = re_dict.get(g)
        if re is None:
            continue
        ...
        if "Group" in re.index:
            preds_full[pos_map[ix]] += float(re["Group"])
        elif "Intercept" in re.index:
            preds_full[pos_map[ix]] += float(re["Intercept"])
        ...
        for name in re.index:
            ...
            if name in df.columns:
                preds_full[pos_map[ix]] += float(re[name]) * float(df.loc[ix, name])
...
def fit_mixed_effects_per_segment(df_long: pd.DataFrame):
    ...
    for seg in df_long["segment"].unique():
        ...
        # Ensure perfect alignment of endog/exog/groups after expansion
        df_fit = df_fit.dropna(subset=[COL_DIAM, 'time_years', COL_HEIGHT, COL_PATIENT]).reset_index(drop=True)
        md = smf.mixedlm(
            "diameter_cm ~ time_years + height_cm",
            df_fit,
            groups=df_fit[COL_PATIENT],
...
def plot_patient(df_long, patient_id, segment, save_dir, show_mixed=True, show_ols=True):
    """Plot a single patient×segment trajectory.

    Flagged outliers (any_outlier_flag, when present) are excluded from:
      - OLS fitting
      - MixedLM fitted line visualization
      - pooled/best-model visualization (if you later add it here)
    Flagged points shown as hollow markers for audit.
    """
    segment = str(segment).lower().strip()
    sub = df_long[
        (df_long[COL_PATIENT] == patient_id)
        & (df_long["segment"].astype(str).str.lower().str.strip() == segment)
    ].copy()

    if sub.empty:
        return

    sub["study_date"] = pd.to_datetime(sub["study_date"], errors="coerce")
    sub = sub.dropna(subset=["study_date", "diameter_cm"])
    if sub.empty:
        return

    # Split usable vs flagged
    if "any_outlier_flag" in sub.columns:
        is_flag = sub["any_outlier_flag"].fillna(False).astype(bool)
        sub_fit = sub.loc[~is_flag].copy()
        sub_out = sub.loc[is_flag].copy()
    else:
        sub_fit = sub.copy()
        sub_out = sub.iloc[0:0].copy()

    if sub_fit.empty and sub_out.empty:
        return

    sub_fit = sub_fit.sort_values("study_date")
    sub_out = sub_out.sort_values("study_date")

    # NOTE: save_dir already encodes segment + ge3/le2 (don’t append segment again)
    save_dir = Path(save_dir)
    save_dir.mkdir(parents=True, exist_ok=True)
    out_path = save_dir / f"patient_{int(patient_id)}_{segment}.png"
    ...
    if not sub_out.empty:
        ax.scatter(
            ...
            edgecolors="k",
            linewidths=1.2,
            label="Flagged outlier (excluded)",
        )

    # MixedLM fitted line (use usable rows only)
    if show_mixed and (not sub_fit.empty) and ("fitted_diameter_cm" in sub_fit.columns):
        subm = sub_fit.dropna(subset=["fitted_diameter_cm"]).sort_values("study_date")
        if subm.shape[0] >= 2:
            ax.plot(
                subm["study_date"],
                subm["fitted_diameter_cm"],
                linewidth=2,
                alpha=0.8,
                label="MixedLM fitted (usable only)",
            )

    # OLS line (fit on usable rows only)
    if show_ols and (not sub_fit.empty) and ("time_years" in sub_fit.columns):
        subo = sub_fit.dropna(subset=["time_years", "diameter_cm"]).copy()
        if subo.shape[0] >= 2:
            x = subo["time_years"].astype(float).values
            y = subo["diameter_cm"].astype(float).values
            slope, intercept = np.polyfit(x, y, 1)

            tmin, tmax = float(np.nanmin(x)), float(np.nanmax(x))
            tgrid = np.linspace(tmin, tmax, 50)
            ygrid = intercept + slope * tgrid

            # Map back to dates using first usable date as origin
            d0 = subo["study_date"].min()
            date_grid = [d0 + pd.to_timedelta(tt * 365.25, unit="D") for tt in tgrid]
            ...
            ax.plot(
                ...
                linestyle="--",
                linewidth=2,
                label="OLS fit (usable only)",
            )

    ax.set_title(f"Patient {patient_id} – {segment}")
...
def generate_all_plots(df_long, save_dir=PLOTS_DIR, min_points_high=3):
    """Generate plots for all patients × segments, separated by point density.

    Output structure:
      <save_dir>/<segment>/ge3/   (>=3 usable points)
      <save_dir>/<segment>/le2/   (<=2 usable points)

    'Usable points' exclude rows flagged by any_outlier_flag when present.
    """
    save_dir = Path(save_dir)
    save_dir.mkdir(parents=True, exist_ok=True)

    d = df_long.copy()
    if "any_outlier_flag" in d.columns:
        d["_usable_for_fit"] = ~d["any_outlier_flag"].fillna(False).astype(bool)
    else:
        d["_usable_for_fit"] = True

    patients = d[COL_PATIENT].dropna().unique()
    segments = d["segment"].dropna().astype(str).str.lower().str.strip().unique()

    # usable point counts per patient×segment
    usable_counts = (
        d.loc[d["_usable_for_fit"]]
        .groupby([COL_PATIENT, "segment"])["diameter_cm"]
        .size()
        .to_dict()
    )

    for seg in segments:
        seg_dir = save_dir / str(seg)
        (seg_dir / "ge3").mkdir(parents=True, exist_ok=True)
        (seg_dir / "le2").mkdir(parents=True, exist_ok=True)

        for pid in patients:
            n_usable = usable_counts.get((pid, seg), 0)
            subdir = seg_dir / ("ge3" if n_usable >= min_points_high else "le2")
            plot_patient(d, pid, seg, subdir)
...
def make_high_risk_files_from_long(
    ...
    # Exclude flagged outliers from high-risk computations
    if "any_outlier_flag" in d.columns:
        d = d.loc[~d["any_outlier_flag"].fillna(False).astype(bool)].copy()
        
    # n measurements (confidence indicator)
    n_meas = d.groupby(["patient_id", "segment"])["diameter_cm"].size().rename("n_meas")

    # Attach measurement counts and high-confidence indicator
    d = d.merge(n_meas.reset_index(), on=["patient_id", "segment"], how="left")
    d["high_confidence"] = np.where(d["n_meas"] >= 3, "✓", "")
